...
orderid_dhltracking_dict = {}
shipdate_Dhltracking_dict = {}
shipdate_Orderid_dict = {}

# Define a function for validating an Email formatting
...
def check(email):
    ...
    if (re.search(regex, email)):
        ...
        return False
#Defining a function to check the availabilty of user input in customer database
...
def verifyUsersExistence(userid):
    ...
    if(customer_list != userid):
        return False
    else:
        return True
#Function to get the status of placed order from Shop API
def fetch_orders(email):
        shop_api_url = "https://demo7609961.mockable.io/orders/"

        #defining my_customer dictionary for the parameters to be sent to the API
